...
// 说明：这些静态成员如果在头文件里“直接定义”，一旦被多个 .cpp 包含就会出现重复链接错误（ODR 违规）。
// C++17 起支持 `inline` 变量：可以安全地把“定义”放在头文件里。
...
// 兜底：如果用户忘了手动 StopWaveEffect()，程序退出时也尽量不要因为 joinable thread 而直接崩溃。
// 注意：这个析构会在“本翻译单元的静态对象销毁阶段”运行。
inline struct WaveEffectAutoStop {
	~WaveEffectAutoStop() noexcept {
		// StopWaveEffect 内部会 join 线程并清理窗口
